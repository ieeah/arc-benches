...
- **State Management**: Zustand per lo stato client e le slice di dominio. Persistenza esplicita tramite adapter isolato [safeStorage.ts](../src/lib/safeStorage.ts) (`safeLS`).
...
- Il tipo generico `List` in [types.ts](../src/types.ts) unifica i banchi di gioco (`listType: 'workbench'`) e le liste custom (`custom: true`, `shared: boolean`).
...
- I selettori dello store vivono come funzioni pure in [selectors.ts](../src/store/selectors.ts).
...
- **Drawer**: apertura coerente con il trigger (`from="top"` per trigger in alto, `from="bottom"` per trigger in basso) integrata con l'hook [useDialog](../src/hooks/useDialog.ts).
