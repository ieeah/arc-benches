...
- **Persistence Boundary Unico**: unificazione dei salvataggi Zustand in un middleware/adapter unico per eliminare le chiamate inlined `saveProfileState` nelle action in [store.ts](file:///c:/Users/flabianca/Projects/Personali/arc-benches/src/store.ts).
